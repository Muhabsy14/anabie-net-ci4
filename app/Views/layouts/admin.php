...
<html lang="id">
<head>
<?= view('partials/head', ['title' => $title ?? 'Admin - Anabie Net']) ?>
</head>
...
<p class="text-xs text-on-primary/60 truncate">Management Console</p>
</div>
</div>
...
$menus = [
    ...
    'laporan'    => ['url' => "/{$prefix}/laporan", 'icon' => 'assessment', 'label' => 'Laporan Operasional'],
...
<p class="text-sm font-semibold text-on-surface truncate lg:hidden">Anabie Net Admin</p>
<p class="hidden lg:block text-sm text-on-surface-variant leading-snug">
Sistem Manajemen WiFi — <strong class="text-secondary">Anabie Net</strong>, Kec. Warungasem, Kab. Batang
</p>
</div>
<a href="<?= base_url(($panelPrefix ?? 'admin') . '/profil') ?>" class="flex items-center gap-2 shrink-0 pl-2 lg:pl-4 border-l border-outline-variant">
...
<div class="w-10 h-10 rounded-full bg-secondary flex items-center justify-center text-on-secondary font-bold"><?= strtoupper(substr($user['nama'] ?? 'A', 0, 1)) ?></div>
</a>
</header>
